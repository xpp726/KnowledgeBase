...
分层：本模块做业务编排，状态合法性走 ``doc_states``，ORM 读写走 Document/Folder Repository，
解析-分块-向量化流水线复用 ``application.documents.ingestion``，不重复实现。
...
from app.domain.documents import states as doc_states
from app.application.folders import service as folder_svc
from app.application.documents.ingestion import (
    IngestResult,
    ingest_bytes as _default_ingest_bytes,
    make_doc_id,
    storage_key,
)
from app.domain.ports.file_storage import FileStorage, ObjectNotFoundError
from app.domain.ports.vector_store import VectorStore

logger = logging.getLogger(__name__)
settings = get_settings()

# 默认实现由组合根装配；模块级工厂保留为测试替换点。
get_embedder = create_embedder
get_storage = create_storage
get_vectorstore = create_vector_store
ingest_bytes = _default_ingest_bytes
...
async def register_document(
    ...
    *,
    ...
    async with create_uow(auto_commit=True) as uow:
        documents = uow.documents
        folders = uow.folders
        knowledge_bases = uow.knowledge_bases
        # 1. 确保 kb 存在
        await knowledge_bases.ensure(
            kb_id,
            name=settings.default_kb_name if kb_id == settings.default_kb_id else kb_id,
        )
        # 2. folder 解析：None → 默认 folder
        if folder_id is None:
            df = await folders.ensure_default(kb_id)
            folder_id = df.folder_id
        else:
            folder = await folders.get_by_id(folder_id)
            if folder is None:
        ...
        same_name = await documents.find_by_folder_name(kb_id, folder_id, file_name)
        ...
        doc = await documents.get_by_id(doc_id)
        if doc is not None:
...
async def process_bytes(
    ...
    *,
    ...
    """
    return await _get_ingest_bytes()(
        data,
        file_name,
        doc_id=doc_id,
        folder_id=folder_id,
        ...
        store=store or _get_vectorstore(),
        storage=storage or _get_storage(),
        force=force,
    )
...
async def mark_document_queued(doc_id: str) -> None:
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        if doc is None or doc.status == doc_states.PENDING:
            return
        doc_states.assert_transition(doc.status, doc_states.PENDING)
        await documents.upsert(doc_id=doc_id, status=doc_states.PENDING)
...
async def move_documents(doc_ids: list[str], target_folder_id: str) -> list[dict]:
    ...
    async with create_uow() as uow:
        documents = uow.documents
        folders = uow.folders
        target = await folders.get_by_id(target_folder_id)
        if target is None:
        ...
        for doc_id in doc_ids:
            ...
            if doc.folder_id == target_folder_id:
                ...
                continue
            dup = await documents.find_by_folder_name(
                target.kb_id, target_folder_id, doc.file_name
            )
            if dup is not None:
                results.append(
                    {
                ...
                continue
            await documents.move_to_folder(doc_id, target_folder_id)
            results.append({"doc_id": doc_id, "status": "moved"})
        await uow.commit()
    return results
...
async def delete_document(
    ...
    *,
    ...
    try:
        async with create_uow(auto_commit=True) as uow:
            documents = uow.documents
            await documents.delete_rows(doc_id)
...
async def recover_stuck(timeout_seconds: int | None = None) -> list[str]:
    ...
    async with create_uow(auto_commit=True) as uow:
        documents = uow.documents
        stuck = await documents.list_stuck(set(doc_states.IN_PROGRESS), before)
        for doc in stuck:
            old = doc.status
            doc_states.assert_transition(old, doc_states.FAILED)
            await documents.upsert(
                doc_id=doc.doc_id,
                status=doc_states.FAILED,
                error=f"进程中断：{old} 状态超过 {timeout}s 未推进，启动时判定卡死，可重试",
            )
...
async def read_document_file(doc_id: str) -> tuple[dict, bytes]:
    ...
    try:
        data = await asyncio.to_thread(_get_storage().get, doc["file_path"])
...
async def list_documents(
    ...
    async with create_uow(auto_commit=True) as uow:
        documents = uow.documents
        rows = await documents.list_by_status(kb_id=kb_id, status=status)
...
async def count_documents_by_status(*, kb_id: str | None = None) -> dict:
    ...
    async with create_uow(auto_commit=True) as uow:
        documents = uow.documents
        counts = await documents.count_by_status(kb_id=kb_id)
    return {
...
async def list_documents_paginated(
    *,
    ...
    async with create_uow(auto_commit=True) as uow:
        documents = uow.documents
        rows, total = await documents.list_page(
            kb_id=kb_id,
            status=status,
            search=search,
            folder_id=folder_id,
...
async def list_knowledge_bases() -> list[dict]:
    async with create_uow(auto_commit=True) as uow:
        knowledge_bases = uow.knowledge_bases
        kbs = await knowledge_bases.list_all()
        counts = await knowledge_bases.count_documents()
...
async def create_knowledge_base(name: str, description: str = "") -> dict:
    ...
    async with create_uow(auto_commit=True) as uow:
        knowledge_bases = uow.knowledge_bases
        folders = uow.folders
        kb = await knowledge_bases.create(kb_id, name=name, description=description)
        await folders.ensure_default(kb.kb_id, name=settings.default_kb_folder_name)
        return _kb_dict(kb)
...
async def ensure_default_knowledge_base() -> None:
    ...
    async with create_uow(auto_commit=True) as uow:
        knowledge_bases = uow.knowledge_bases
        folders = uow.folders
        kb = await knowledge_bases.ensure(
...
async def migrate_orphan_documents() -> int:
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        folders = uow.folders
        # 先列所有有孤儿文档的 kb_id
        orphan_kbs = await documents.list_orphan_kb_ids()
        for kb_id in orphan_kbs:
            ...
            df = await folders.ensure_default(kb_id, name=settings.default_kb_folder_name)
            # 把孤儿文档归到默认 folder
            n = await documents.assign_folder_for_orphans(kb_id, df.folder_id)
            migrated += n
            if n > 0:
