"""文档入库 Application 流水线（Web 上传与 CLI 批量导入共用）。
...
logger = logging.getLogger("app.application.documents.ingestion")
settings = get_settings()

# 默认实现由组合根装配。模块级名称保留给迁移期脚本和测试 monkeypatch 使用。
get_storage = create_storage
parse_file = parse_document
chunk_document = chunk_parsed_document
...
async def _mark(doc_id: str, target: str, **fields) -> None:
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        current = await documents.status(doc_id)
        doc_states.assert_transition(current, target)
        await documents.upsert(doc_id=doc_id, status=target, **fields)
...
async def ingest_bytes(
    ...
    *,
    ...
    parser = parser or parse_file
    chunker = chunker or chunk_document
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        if not force:
            st = await documents.status(doc_id)
            if st == "done":
    ...
    try:
        ...
        parsed = await asyncio.to_thread(parser, tmp_path, doc_id)
        if not parsed.ok:
        ...
        # ---- 3. 线程池分块（CPU 密集）----
        chunks = await asyncio.to_thread(chunker, parsed)
        if not chunks:
            err = "未分出任何 chunk"
