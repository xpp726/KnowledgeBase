"""会话业务编排：会话获取/创建、多轮历史、消息与查询日志落库。

分层约定：本模块只做业务编排（标题生成、refs 序列化、历史裁剪、同时写
message 与 query_log），具体 ORM 读写收拢在 ConversationRepository；不 import FastAPI。
...
async def record_assistant_turn(
    ...
    *,
    ...
    async with create_uow() as uow:
        ...
        await conversations.add_query_log(
            log_id=_id("l_"),
            conversation_id=conversation_id,
            kb_id=kb_id or settings.default_kb_id,
            mode=mode,
...
async def conversation_messages(conversation_id: str) -> list[dict]:
    ...
    async with create_uow() as uow:
        conversations = uow.conversations
        rows = await conversations.list_messages(conversation_id)
        out = []
        for m in rows:
            try:
...
async def remove_conversation(conversation_id: str) -> None:
    async with create_uow() as uow:
        conversations = uow.conversations
        await conversations.delete_rows(conversation_id)
        await uow.commit()
...
async def rename_conversation(conversation_id: str, title: str) -> dict | None:
    ...
    async with create_uow() as uow:
        conversations = uow.conversations
        conv = await conversations.rename(conversation_id, title)
        await uow.commit()
