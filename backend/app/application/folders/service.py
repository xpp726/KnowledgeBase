...
class FolderApplicationService:
    """文件夹只读查询用例，供 API 依赖注入使用。"""

    def __init__(self, uow_factory: Callable[[], ReadUnitOfWork]):
        self._uow_factory = uow_factory

    async def get_by_id(self, folder_id: str) -> FolderRecord | None:
        async with self._uow_factory() as uow:
            return await uow.folders.get_by_id(folder_id)
    ...
    async def find_sibling(
        ...
        *,
        ...
        async with self._uow_factory() as uow:
            return await uow.folders.find_sibling(
                kb_id=kb_id,
                parent_id=parent_id,
                name=name,
            )
...
def _folder_dict(f: FolderRecord) -> dict:
    return {
...
def _folder_to_node(f: FolderRecord, doc_count: int = 0) -> FolderNode:
    return FolderNode(
        folder_id=f.folder_id,
        kb_id=f.kb_id,
        parent_id=f.parent_id,
...
def build_tree(folders: list[FolderRecord], doc_count_map: dict[str | None, int]) -> list[FolderNode]:
...
async def ensure_default_for_kb(kb_id: str, name: str = "默认文件夹") -> dict:
    ...
    async with create_uow(auto_commit=True) as uow:
        folders = uow.folders
        f = await folders.ensure_default(kb_id, name=name)
...
async def create_folder(
    *, kb_id: str, parent_id: str | None, name: str
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        await folders.ensure_knowledge_base(kb_id, name=kb_id)

        # 计算 depth 与校验
        if parent_id is None:
            depth = 1
        else:
            parent = await folders.get_by_id(parent_id)
            if parent is None:
        ...
        # 同名校验
        existed = await folders.find_sibling(kb_id=kb_id, parent_id=parent_id, name=name)
        if existed is not None:
            raise FolderNameConflictError(name, parent_id)

        f = await folders.create(
            kb_id=kb_id, parent_id=parent_id, depth=depth, name=name
        )
...
async def rename_folder(folder_id: str, new_name: str) -> dict:
    ...
    async with create_uow(auto_commit=True) as uow:
        folders = uow.folders
        f = await folders.get_by_id(folder_id)
        if f is None:
            raise FolderNotFoundError(folder_id)
        # 同名校验（除自身外）
        conflict = await folders.find_sibling(
            kb_id=f.kb_id, parent_id=f.parent_id, name=new_name
        )
        if conflict is not None and conflict.folder_id != folder_id:
            raise FolderNameConflictError(new_name, f.parent_id)
        f = await folders.rename(folder_id, new_name)
...
async def move_folder(folder_id: str, new_parent_id: str | None) -> dict:
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        if new_parent_id is not None:
            ...
            descendants = await folders.collect_descendants([folder_id])
            if new_parent_id in descendants:
                raise FolderError("不能将文件夹移到自身子孙目录下")
            new_parent = await folders.get_by_id(new_parent_id)
        ...
        f = await folders.move(folder_id, new_parent_id)
        # 移动后，本 folder + 所有子孙的 depth 都要重新计算
        await folders.propagate_depths(folder_id, new_depth)
...
async def get_folder_tree(kb_id: str) -> list[dict]:
    ...
    async with create_uow(auto_commit=True) as uow:
        folder_repo = uow.folders
        folder_models = await folder_repo.list_all(kb_id)
        counts = await folder_repo.count_documents(kb_id)
        tree = build_tree(folder_models, counts)
...
async def get_folder_path_map(kb_id: str) -> dict[str, FolderNode]:
    ...
    async with create_uow(auto_commit=True) as uow:
        folder_repo = uow.folders
        folder_models = await folder_repo.list_all(kb_id)
    return {f.folder_id: _folder_to_node(f, 0) for f in folder_models}
...
async def delete_folder_cascade(folder_id: str) -> DeleteFolderReport:
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        all_folder_ids = await folders.collect_descendants([folder_id])
        report.cascaded_folder_ids = all_folder_ids

        # 取所有要删除的 doc_id
        doc_ids = await folders.list_document_ids(all_folder_ids)
    ...
    async with create_uow(auto_commit=True) as uow:
        ...
        # 已兜底，这里再保险清一次）
        await folders.unset_documents(doc_ids)
        await folders.delete_rows(report.cascaded_folder_ids)
